...
# define clients
def get_openai_client(sync: bool = False):
    if not OPENAI_API_KEY:
        raise ValueError("OPENAI_API_KEY is not set. Please provide it via environment or .env file.")

    if sync:
        http_client = httpx.Client(proxy=OPENAI_PROXY) if OPENAI_PROXY else httpx.Client()
        return openai.Client(api_key=OPENAI_API_KEY, http_client=http_client)

    http_client = httpx.AsyncClient(proxy=OPENAI_PROXY) if OPENAI_PROXY else httpx.AsyncClient()
    return openai.AsyncClient(api_key=OPENAI_API_KEY, http_client=http_client)
...
async def filter_synergies(
    product_A: str,
    product_B: str,
    citations: list[str],
    synergy_threshold: float = SYNERGY_THRESHOLD,
    verbose: bool = False,
) -> tuple[list[tuple[float, str, str]], int]:
    synergies = []
    descriptions_retrieved = 0
    for citation_url in tqdm(citations, desc='Processing citations synergy'):
        ...
        if synergy_o3 and (synergy_gpt > synergy_threshold):
            ...
            if verbose:
                preview_text = " ".join(citation_content.split())[:50]
                print(f"Synergy case for {product_A} -> {product_B}: {citation_url} | {preview_text}")

    return synergies, descriptions_retrieved
# check synergy
...
# product matching
async def process_product(product_A: str, product_B: str, synergy_threshold: float = SYNERGY_THRESHOLD, verbose: bool = False):

    default_result = {
        'best_relevance': 0,
        'best_relevance_citation': '',
        'best_relevance_citation_url': '',
        'best_synergy': 0,
        'best_synergy_citation': '',
        'best_synergy_citation_url': '',
        'case_count': 0,
        'description_count': 0,
    }
    ...
    try:
      ...
      if verbose and have_cases:
         print(citations)
      default_result['cases'] = citations
      case_count = len(citations) if citations else 0
      default_result['case_count'] = case_count


      if verbose:
          print(f"Found cases for {product_A} -> {product_B}: {citations}")
      ...
      synergies, descriptions_retrieved = await filter_synergies(product_A, product_B, citations, synergy_threshold, verbose=verbose)
      default_result['synergies'] = synergies
      default_result['description_count'] = descriptions_retrieved

      if verbose:
          print(f"Descriptions retrieved: {descriptions_retrieved}")
...
async def process_row(product_A, row, synergy_threshold: float = SYNERGY_THRESHOLD, verbose: bool = False):

    async with product_semaphore:

        # dimension reduction
        product_B = row['Direction']
        result = await process_product(product_A, product_B, synergy_threshold, verbose=verbose)
        return (row.name, result)

async def process_all_rows(product_A: str, df: pd.DataFrame, synergy_threshold: float = SYNERGY_THRESHOLD, verbose: bool = False):

    tasks = [process_row(product_A, row, synergy_threshold, verbose=verbose) for _, row in df.iterrows()]
...
df = pd.read_excel("data/radar-set.xlsx")
...
async def compute_molecule(product_ru: str, df: pd.DataFrame = df, SYNERGY_THRESHOLD: float = 0.95, TEST: bool = False, verbose: bool = False) -> pd.DataFrame:
    working_df = df.loc[:10].copy() if TEST else df.copy()
    ...
    results = await process_all_rows(product_A, output_df, SYNERGY_THRESHOLD, verbose=verbose)
    ...
    for i, result in results:
        ...
        description_count = result.get('description_count', 0)
